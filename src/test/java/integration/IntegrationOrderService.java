...
class IntegrationOrderService extends JpaH2TestBase {
    ...
    private Stato statoBdiversoDa(Stato a) {
        ...
        return a; // fallback (se enum avesse 1 solo valore)
    }
    ...
    void setUp() {
        ...
        // ✅ Pulizia forte: prima tabella ElementCollection, poi Ordine
        // Nome tabella tipico Hibernate: Ordine_items
        try { em.createNativeQuery("DELETE FROM Ordine_items").executeUpdate(); } catch (Exception ignored) {}
        try { em.createQuery("delete from Ordine").executeUpdate(); } catch (Exception ignored) {}

        // se GestoreOrdini/Utente non sono mappati nel PU test, queste delete non servono (e possono fallire)
        try { em.createQuery("delete from GestoreOrdini").executeUpdate(); } catch (Exception ignored) {}
