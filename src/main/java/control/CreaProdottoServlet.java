...
public class CreaProdottoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Utente loggedUser = (session != null) ? (Utente) session.getAttribute("loggedUser") : null;

        if (loggedUser == null || loggedUser.getRuolo() != Ruolo.FORNITORE) {
            response.sendError(403, "Solo un fornitore può creare prodotti.");
            return;
        }

        Long fornitoreId = loggedUser.getId();

        Prodotto prodotto = new Prodotto(nome, descrizione, prezzo, null, categoria, 0, false, false, fornitoreId);
