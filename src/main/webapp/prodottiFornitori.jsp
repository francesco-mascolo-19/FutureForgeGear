  ...
  <script>
    ...
    function openRequestForm(id, name, idFornitore, idProd) {
      ...
      document.getElementById('request-form').onsubmit = function(event) {
        ...
        params.append("note", note); // opzionale
        ...
        fetch("RichiestaProdottoServlet", {
          ...
          body: params
                ...
                .then(async (response) => {
                  ...
                  if (!response.ok) {
                    alert("Errore invio richiesta: " + text);
                    return;
                  }
                  alert("Richiesta inviata: " + text);
                  closeRequestForm();
                })
                .catch(error => console.error("Errore:", error));
      };
...
<table>
  ...
  <tr>
    ...
    <td>
      <%
        Integer id = p.getId();
        Long fid = p.getFornitore();

        String safeName = p.getNome();
        if (safeName == null) safeName = "";
        safeName = safeName.replace("\\", "\\\\").replace("'", "\\'");
      %>

      <button class="btn-edit"
              onclick="openRequestForm(<%= id %>, '<%= safeName %>', <%= fid %>, <%= id %>)">
        Richiesta
      </button>
